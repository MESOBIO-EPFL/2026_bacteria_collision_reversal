...
function [bctAllN,mvBndN]=bct_iterationFxMvBnd...
    (bctAll,fxBnd,mvBnd,nePr,nePrFxBnd,nePrMvBnd,mp)

%% Compute forces (passive and active) and moments (passive and active)
[frc,mnt,bctAllN]=bct_forceComputeAll(bctAll,nePr,mp);
[frcFxBnd,mntFxBnd,bctAllN]=...
    bct_forceComputeFxBndAll(bctAllN,fxBnd,nePrFxBnd,mp);
[frcMvBnd,mntMvBnd,mvBndFrc,bctAllN]=bct_forceComputeMvBndAll...
    (bctAllN,mvBnd,nePrMvBnd,mp);
...
bctAllN(:,1:2)=bctAllN(:,1:2)+...
    1./mp.tmSclFrt.*(frc+frcAct+frcFxBnd+frcMvBnd).*mp.tmSt;
bctAllN(:,3)=bctAllN(:,3)+1/mp.tsOrn./mp.tmSclFrt.*...
    (mnt+mntFxBnd+mntMvBnd+bctAllN(:,5)).*mp.tmSt;
...
%% Update moving boundary position
mvBndN=mvBnd+mvBndFrc./mp.tsBnd.*mp.tmSt;
if mp.prBndX==1
    mvBndN(:,1)=mod(mvBndN(:,1),mp.lx);
end

if mp.prBndY==1
    mvBndN(:,2)=mod(mvBndN(:,2),mp.ly);
end
