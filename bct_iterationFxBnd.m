...
function bctAllN=bct_iterationFxBnd(bctAll,fxBnd,nePr,nePrFxBnd,mp)

%% Compute forces (passive and active) and moments (passive and active)
[frc,mnt,bctAllN]=bct_forceComputeAll(bctAll,nePr,mp);
[frcBnd,mntBnd,bctAllN]=bct_forceComputeFxBndAll(bctAllN,fxBnd,nePrFxBnd,mp);
...
bctAllN(:,1:2)=bctAllN(:,1:2)+1./mp.tmSclFrt.*(frc+frcAct+frcBnd).*mp.tmSt;
bctAllN(:,3)=bctAllN(:,3)+1/mp.tsOrn./mp.tmSclFrt.*...
    (mnt+mntBnd+bctAllN(:,5)).*mp.tmSt;
